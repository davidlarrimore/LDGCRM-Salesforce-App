...
description: Safety checklist for running or modifying any script that deletes, hard-deletes, or bulk-modifies records in a GSA PEO org (Dev sandbox by default) (e.g. tools/data-loading/Invoke-SandboxFactoryReset.ps1, future data-migration loads).
---
...
`tools/data-loading/Common.Orgs.ps1`. Never pass a bare `--target-org`/`-OrgAlias` to reach a registered
...
2. **Preflight with a read-only count first.** Show the user how many records will be affected
   ...
   `tools/data-loading/Invoke-SandboxFactoryReset.ps1` already does this; keep the pattern in any new script.
...
Every destructive/write script gates on `Assert-LdgcrmTypedConfirmation` (`tools/data-loading/Common.ps1`):
...
Anything added under `tools/data-loading/` that upserts, updates, or deletes at scale should
follow the same pattern: confirm org, preflight count, export-before-write, explicit typed
confirmation for anything irreversible, and logging via `tools/data-loading/Common.ps1` into
`logs/data-migration/`.
