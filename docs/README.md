...
**The order of operations for a deployment**, across every team with a hand in it: deploy the
metadata, activate the Flows, provision users and sharing, then load the data.
`PRODUCTION-READINESS.md` says whether we are ready; that says what actually happens, in what order,
and who does it.
