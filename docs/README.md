...
| `Build-OpportunityPartnerAccountLink.ps1` | Prep — Opportunity second pass, sets `LDGCRM_Partner_Account__c` only | Built and **loaded 2026-08-13: 66/66**. Derived from Applications (the only authored path); 16 pending until both sides load, 0 conflicts. Deliberately does **not** use Partner Accounts' `Opportunities` column — that's an Account-level rollup, not a real link. |
