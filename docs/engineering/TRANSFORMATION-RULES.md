...
| **An Airtable Account row carrying no Opportunities, Partner Accounts or Applications does not need matching** | 2026-08-16 | Contacts and Meetings reach Salesforce *through* those three, so a row carrying only them costs nothing on its own and is not a review item. **Score the rule on `Opportunities` + `Partner Agencies` only** — the Accounts table's `Programs` column is not Applications (those IDs belong to a `Programs` table no transform reads), and Applications already reach an Account through its Partner Accounts. ⚠️ **`@($null).Count` is 1**: Airtable omits empty fields entirely, so `@($rec.fields.Opportunities).Count` returns 1 for a row with none and overstates the whole population — filter first (`| Where-Object { $_ }`). That produced a confident, wrong answer once. |
