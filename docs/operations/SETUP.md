...
`scripts/common/Common.Orgs.ps1`. **You never pass a Salesforce username or org alias by hand.**
...
`scripts/common/Common.Orgs.ps1` **first**, then authenticate. The registry is what the identity
...
```powershell
powershell scripts/data-migration/Get-AirtableExport.ps1 -Tables "Impediments"
```
