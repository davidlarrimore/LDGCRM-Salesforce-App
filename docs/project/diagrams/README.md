...
global counter — each document starts again at `01`. The `.png` and `.html` of one diagram share a
stem, so they sort next to each other and neither can be matched to the wrong partner.
...
| `technical-deployment-plan-` | `../Login.gov Partnerships CRM Technical Deployment Plan - DRAFTED.docx` |
...
| File | Source | Appears as |
| --- | --- | --- |
| `technical-deployment-plan-01-deployment-process.png` | `technical-deployment-plan-01-deployment-process.html` | Figure 1, after the Introduction |

`style.css` is shared by every source.

### What the other document holds, so nobody re-checks

The **Release Plan** has no images at all.
...
## The filenames inside a `.docx` are the OLD, unprefixed ones, on purpose

Both documents' `word/media/` entries carry the name without the document prefix —
`01-object-model.png`, `01-deployment-process.png`. The prefix is a *repository* convention; inside
the package the names only have to be unique, and prefixing them would mean editing
`word/_rels/document.xml.rels` for no gain. Strip the prefix from a filename here to get its name in
the package.
...
width the documents place it at. Match `--window-size` to the SVG's own `width`/`height` — the scale
...
| `technical-deployment-plan-01-deployment-process.html` | `1560,444` | 4680 x 1332 |
...
## Putting a re-rendered diagram back into a document

A `.docx` is an ordinary OOXML package: the PNGs sit in `word/media/` under their unprefixed names.
Replacing one in place is enough, **provided the new PNG has the same aspect ratio** — each image is
fixed at 6.5 inches wide with an explicit height in EMU, so a differently-shaped PNG is stretched
...
## Adding a NEW figure to a document

Four parts have to change together, and Word reports a generic "unreadable content" error if any one
of them is missed:

1. `[Content_Types].xml` — a `<Default ContentType="image/png" Extension="png"/>` entry. A document
   that has only ever held a JPEG will not have one.
2. `word/_rels/document.xml.rels` — a new `Relationship` of type `.../image` pointing at
   `media/<name>.png`. Pick an `Id` no other relationship uses.
3. `word/document.xml` — the figure paragraph and its caption paragraph, using the `r:embed` id from
   step 2 and both `cx`/`cy` pairs from the formula above.
4. `word/media/<name>.png` — the image itself.

**Write every XML part back as UTF-8 with NO byte-order mark.** A BOM in front of the XML
declaration makes the part unparseable, and `XmlDocument.Save()` adds one, which is why these edits
are done as string replacements rather than through an XML DOM.

The figure and caption markup is identical in both documents; copy an existing pair rather than
