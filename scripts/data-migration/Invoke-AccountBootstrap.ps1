...
<#
    ...
    production Account export (data/peo-prod-accounts-<date>.xls).
...
param(
    ...
    # Defaults to the newest data/peo-prod-accounts-*.xls.
    [string]$SourceFile = "",
...
if (-not $SourceFile) {
    throw ("No production Account export found. Expected data/peo-prod-accounts-<yyyy-MM-dd>.xls " +
           "(a Salesforce report export saved as HTML with an .xls extension).")
}
