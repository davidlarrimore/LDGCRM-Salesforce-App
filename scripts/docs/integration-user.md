...
| Permission set | **LDGCRM_Partner_Portal_API_R** ("LDGCRM - Partner Portal API - R") |
...
`LDGCRM_Partner_Portal_API_R` grants four user permissions:
...
→ add **LDGCRM - Partner Portal API - R** → Save.
...
`LDGCRM_Issuer_String__c` carries **no field grants, and cannot**. Its issuer
string field is **required**, and Salesforce refuses field-level security on a
required field (*"You cannot deploy to a required field"*): object read already
makes it readable. `Name` is an Auto Number and the Master-Detail is required
too, so object read covers every field on the object.
...
| | |
| --- | --- |
| `Name` | **Auto Number**, `ISID-{00000}`, label *Issuer String ID* — carries no data |
| `LDGCRM_Issuer_String__c` | **Text(200)**, required, **not unique** — the issuer string |
| `LDGCRM_Application__c` | **Master-Detail** to `LDGCRM_application__c`, reparentable |
| Sharing | `ControlledByParent` |
| External ID field | **none, deliberately** — the string is the only data |
| Apex triggers | none |
| Records in Dev | **2,598, awaiting a reload** |

The 2,598 records in Dev predate the Auto Number conversion (2026-09-17, CLI
deploy to Dev), and they are **one 866-record load run three times** (2026-09-10,
and twice on 2026-09-14) — so every issuer string appears three times on its
Application. **The conversion kept each existing `Name`**, so the issuer string
text is still there, in the field now labelled *Issuer String ID*; only records
created afterwards get `ISID-` numbers. `LDGCRM_Issuer_String__c` is blank on all
2,598. They are to be deleted and reloaded. Rules for loading
the object: `docs/engineering/TRANSFORMATION-RULES.md`.
