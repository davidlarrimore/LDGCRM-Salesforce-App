...
# Id. Kept beside the rows, not on them - anything added to the output row
# becomes a column in the CSV sent to Salesforce.
$OwnerSourceByRecId = @{}
...
foreach ($Row in $AirtableApplications) {
    ...
    if (-not $LoadedPartnerAccountIds.Contains($PartnerAccountId)) {
        $SkippedRows.Add([PSCustomObject]@{
            AirtableRecordId = $RecId
            Name             = $Name
            Reason           = "Linked Partner Account $PartnerAccountId is not loaded in $OrgAlias (its own parent Account is unresolved - see docs/TROUBLESHOOTING.md). Would fail the load with INVALID_FIELD - skipped. Re-run once the Account/Partner Account data is fixed."
        })
        continue
    }
...
if ($BrokerParentRows.Count -gt 0) {
    ...
    Write-Host "  scripts\powershell-scripts\Invoke-SalesforceLoad.ps1 -Environment $Environment ``" -ForegroundColor DarkGray
