...
function Get-LdgcrmRoot {
    <#
        THE REPOSITORY ROOT. Every path these tools read or write hangs off it:

            <repo>/.env                    credentials (gitignored)
            <repo>/data/airtable-exports/  pulled Airtable JSON
            <repo>/data/prod-accounts/     the production Account export
            <repo>/data/salesforce-loads/  load-ready CSVs
            <repo>/logs/<category>/        run output
            <repo>/reference/              the Salesforce user roster

        This file is at <repo>/tools/data-loading/Common.ps1, so the root is TWO
        levels up.

        WHY IT MOVED (2026-09-08). Sprint 1 shipped, and with it the idea that
        this code is an operations bundle. It used to live in scripts/ and return
        THAT folder, because the GSA Operations team took the pipeline into their
        own repository as a plain /scripts directory and nothing inside it could
        resolve a path above its own root.

        None of that applies now. These are DEV/QA TOOLS: they rebuild a
        developer sandbox so the app can be worked on, they are not shipped to
        anyone, and they sit in tools/ alongside the metadata tooling for exactly
        that reason. So the constraint that shaped the old layout - self
        containment - is gone, and the data and logs live at the repository root
        where the rest of the engineering tooling already writes.

        The name is unchanged deliberately: 15 call sites across these scripts
        resolve data/, logs/, .env and reference/ through it, and they are all
        correct as written once this function points one level higher.
    #>
...
function Get-LogDirectory {
    ...
    param(
        [Parameter(Mandatory = $true)]
        [ValidateSet("cleanup", "data-migration", "sprint2")]
        [string]$Category
    )
...
function Import-DotEnv {
    <#
        Loads KEY=VALUE pairs from <repo>/.env into the current process's
        environment variables. Skips blank lines and #-comments. Never overwrites
        a variable that's already set in the environment, so an explicit $env:X
        set before running a script still wins.

        Copy .env.example at the repository root to start - that file documents
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        # A stale copy at the OLD location (scripts/.env, where it lived while
        # this was an operations bundle) would otherwise be silently ignored and
        # the operator would see "AIRTABLE_API_KEY not set" with no hint that
        # their credentials are sitting right there. Read-only, and gated on the
        # file actually being ours.
        $Legacy = Join-Path (Join-Path (Get-LdgcrmRoot) "scripts") ".env"
        ...
        if (Test-Path -LiteralPath $Legacy) {
            ...
            try {
                ...
                # "not ours" rather than failing the run on someone else's file.
                $LooksLikeOurs = $false
            }

            if ($LooksLikeOurs) {
                throw ("Found this pipeline's .env at $Legacy, but it now reads $Path. " +
                       "Move the file to the repository root - the dev/QA tools resolve " +
                       "everything from " + (Get-LdgcrmRoot) + ".")
            }
        }
