...
param(
    # LAYER 1 OF THE PRODUCTION BLOCK: only the two developer sandboxes are
    # accepted, and PowerShell rejects anything else at bind time, so there is no
    # code path out of Dev/QA at all.
    #
    # UAT and Full were dropped here on 2026-09-08 along with the rest of the
    # non-sandbox environments: both are COPIES OF PRODUCTION, so their Accounts
    # are the real records, and a factory reset has no business in either. Prod
    # was never in this list. Do not add any of them back - if a production-side
    # operation is ever needed it belongs in a separate tool, not behind a
    # widened ValidateSet on a script whose job is hard-deleting records.
    [ValidateSet("Dev", "QA")]
