...
<#
    ...
    Usage:
        powershell tools/metadata/Sync-Metadata.ps1                  # Dev sandbox
        powershell tools/metadata/Sync-Metadata.ps1 -Environment QA
        powershell tools/metadata/Sync-Metadata.ps1 -WhatIf          # report only; no manifest edit, no retrieve
...
function Update-ManifestDiscovery {
    ...
    if ($NeedsReview.Count -gt 0) {
        ...
        Write-Host "Otherwise, add them to tools/metadata/ldgcrm-manifest-ignore.json so they stop resurfacing." -ForegroundColor Yellow
    }
